...
const RevenueByCategoryTab = () => {
  ...
  const mutateByProduct = useMutation({
    mutationFn: RevenueApis.getByCategory,
    onSuccess: (res, variables) => {
      // console.log('res', res);
      if (variables.type === 'month') {
        const labels: any[] = [];
        const data: any[] = [];
        res?.revenue_by_category?.forEach((item: any, index: number) => {
          if (index <= 5) {
            index == 5
              ? labels.push('Khác')
...
export default RevenueByCategoryTab;
